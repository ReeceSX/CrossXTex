...
namespace
{
    ...
    HRESULT ResizePointFilter(const Image& srcImage, const Image& destImage) noexcept
    {
        ...
        ScopedAlignedArrayXMVECTOR scanline(static_cast<XMVECTOR*>(AllocateVectorAligned(
            (sizeof(XMVECTOR) * (srcImage.width + destImage.width)))));
        if (!scanline)
    ...
    HRESULT ResizeBoxFilter(const Image& srcImage, TEX_FILTER_FLAGS filter, const Image& destImage) noexcept
    {
        ...
        ScopedAlignedArrayXMVECTOR scanline(static_cast<XMVECTOR*>(AllocateVectorAligned(
            (sizeof(XMVECTOR) * (srcImage.width * 2 + destImage.width)))));
        if (!scanline)
            return E_OUTOFMEMORY;
    ...
    HRESULT ResizeCubicFilter(const Image& srcImage, TEX_FILTER_FLAGS filter, const Image& destImage) noexcept
    {
        ...
        ScopedAlignedArrayXMVECTOR scanline(static_cast<XMVECTOR*>(AllocateVectorAligned(
            (sizeof(XMVECTOR) * (srcImage.width * 4 + destImage.width)))));
        if (!scanline)
            return E_OUTOFMEMORY;
        ...
        for (FilterFrom* yFrom = tfY->from; yFrom < yFromEnd; )
        {
            ...
            for (size_t j = 0; j < yFrom->count; ++j)
            {
                ...
                if (!rowAcc->scanline)
                {
                    ...
                    {
                        rowAcc->scanline.reset(static_cast<XMVECTOR*>(AllocateVectorAligned(sizeof(XMVECTOR) * destImage.width)));
                        if (!rowAcc->scanline)
                            return E_OUTOFMEMORY;
                    }
