        ...
        float& exposure) noexcept
    {
        ...
        while (size > 0)
        {
            ...
            else if ((size > exposureLen) && memcmp(info, g_Exposure, exposureLen) == 0)
            {
                ...
                char buff[32] = {0};
                strncpy(buff, info, std::min<size_t>(31, len));
        ...
        uint32_t height = 0;
        if (sscanf(orientation + 2, "%u", &height) != 1)
        {
            return E_FAIL;
        }
...
HRESULT DirectX::GetMetadataFromHDRFile(const wchar_t* szFile, TexMetadata& metadata) noexcept
{
    ...
    if (hFile.GetLength() < sizeof(g_Signature))
    {
        return E_FAIL;
    }
    ...
    uint8_t header[8192] = {};
    DWORD bytesRead = 0;
    if (!(bytesRead = hFile.Read(header, std::min<DWORD>(sizeof(header), hFile.GetLength()))))
    {
        return E_FAIL;
    }
...
HRESULT DirectX::LoadFromHDRFile(const wchar_t* szFile, TexMetadata* metadata, ScratchImage& image) noexcept
{
    ...
    // Read file
    std::unique_ptr<uint8_t[]> temp(new (std::nothrow) uint8_t[length]);
    if (!temp)
    {
    ...
    DWORD bytesRead = 0;
    if (!(bytesRead = hFile.Read(temp.get(), length)))
    {
        return E_FAIL;
    }
    ...
    return LoadFromHDRMemory(temp.get(), length, metadata, image);
}
...
HRESULT DirectX::SaveToHDRMemory(const Image& image, Blob& blob) noexcept
{
    ...
    char header[256] = {};
    sprintf(header, g_Header, image.height, image.width);
...
HRESULT DirectX::SaveToHDRFile(const Image& image, const wchar_t* szFile) noexcept
{
    ...
    else
    {
        ...
        for (size_t scan = 0; scan < image.height; ++scan)
        {
            ...
            if (!hFile.Write(rgbe, static_cast<DWORD>(rowPitch)))
            {
                return E_FAIL;
            }
        ...
        for (size_t scan = 0; scan < image.height; ++scan)
        {
            ...
            if (encSize > 0)
            {
                ...
                if (!hFile.Write(enc, static_cast<DWORD>(encSize)))
                {
                    return E_FAIL;
                }
            }
