...
namespace
{
        ...
        const Image& destImage)
    {
        ...
        ScopedAlignedArrayXMVECTOR scanlines(static_cast<XMVECTOR*>(AllocateVectorAligned((sizeof(XMVECTOR)*width*2))));
        if (!scanlines)
            return E_OUTOFMEMORY;
    ...
    size_t yOffset) noexcept
{
    ...
    ScopedAlignedArrayXMVECTOR scanline(static_cast<XMVECTOR*>(AllocateVectorAligned((sizeof(XMVECTOR)*srcRect.w))));
    if (!scanline)
        return E_OUTOFMEMORY;
